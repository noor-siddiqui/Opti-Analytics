...
class Data_Engine {
	...
	public function get_dashboard_metrics( string $start_date, string $end_date ): array {
		...
		// Optimized Out of stock products count (using paginated total to avoid memory bloat).
		$oos_results             = wc_get_products(
			array(
				'status'       => 'publish',
		...
		$metrics['out_of_stock'] = (float) ( $oos_results->total ?? 0.0 );
