...
class Data_Engine {
	...
	public function get_dashboard_metrics( string $start_date, string $end_date ): array {
		...
		// Optimized Out of stock products count (using wc_get_products pagination).
		// By using limit=1 and paginate=true, we avoid loading unbounded arrays into memory.
		$oos_paginated           = wc_get_products(
			array(
				'status'       => 'publish',
		...
		$metrics['out_of_stock'] = $oos_paginated->total;
